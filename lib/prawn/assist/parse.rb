  ...
  module Assist    
    ...
    class Parse
      ...
      def self.html(html, options = {})
        ...
        if options && options[:strip_tags]
          options[:strip_tags].each do |tag|
            html.strip_tags!(tag)
          end
        end
                
        if options && options[:block_tags]
        ...
        if options && options[:strip_h_tags]
        ...
        result.strip_tags!('p')        
        result.replace_tags_marker!('hr', @@NEW_PAGE)
        result.gsub!(/&rdquo;/, "'")
        result.gsub!(/&ldquo;/, "'")
        result.gsub!(/“/, "'")
        result.gsub!(/”/, "'")
